...
def get_dashboard_metrics(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "risk_level": "Medium",
            "anomalies_detected": [
                f"Database error ({e}), showing mocked health status."
            ],
...
def chat_with_cio(request: QueryRequest):
    ...
    if request.role not in ["CIO", "CTO", "Executive"]:
        raise HTTPException(
            status_code=403, detail="Unauthorized role for Strategic AI insights."
        )
    ...
    try:
        ...
        return {
            "response": f"AI Engine encountered an error: {e}",
